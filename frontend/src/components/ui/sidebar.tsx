...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((v: boolean) => boolean)) => {
      const computedOpen = typeof value === 'function' ? value(open) : value
      if (setOpenProp) setOpenProp(computedOpen)
      else _setOpen(computedOpen)
      document.cookie = `sidebar_state=${computedOpen ? 'true' : 'false'}; path=/; max-age=${60 * 60 * 24 * 365}`
    },
    [setOpenProp, open],
  )
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-mobile="true"
          className="inset-y-0 left-0 w-(--sidebar-width-mobile) border-l-0 border-r bg-sidebar p-0 text-sidebar-foreground data-[state=closed]:-translate-x-6 data-[state=open]:translate-x-0 [&>button]:hidden"
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <button
        ...
        title="切换侧栏"
        className={cn(
          'motion-interactive absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex',
